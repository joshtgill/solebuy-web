...
def category(request):
    ...
    products = Assistant().findProducts(categoryData, request.session['filterIdMap']).get('primary')
    ...
    content = {'categoryForm': categoryForm, 'data': categoryData,
               ...
               'filterIdMap': request.session['filterIdMap'],
               'products': products,
               'product': product}
...
def updateIdMap(request, cleaned_data):
    ...
    filterIdMap = request.session['filterIdMap']
    if filterId not in filterIdMap[assisterId]:
        filterIdMap[assisterId].append(filterId)
    else:
        filterIdMap[assisterId].remove(filterId)

    request.session['filterIdMap'] = filterIdMap


def resetIdMap(request, numAssisters):
    request.session['filterIdMap'] = [[] for i in range(numAssisters)]
